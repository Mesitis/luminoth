...
@click.option(
    "--from-global-step", type=int, default=None, help="Consider only checkpoints after this global step"
)  # noqa
@click.option("override_params", "--override", "-o", multiple=True, help="Override model config params.")  # noqa
@click.option(
    "--files-per-class", type=int, default=10, help="How many files per class display in every epoch."
)  # noqa
...
def eval(dataset_split, config_files, watch, from_global_step, override_params, files_per_class, max_detections):
    ...
    if config.model.type == "ssd" or config.model.network.with_rcnn:
        ...
        pred_objects_classes = tf.zeros((tf.shape(pred_objects_scores)[0],), dtype=tf.int32)
    ...
    while True:
        ...
        try:
            checkpoints = get_checkpoints(run_dir, last_global_step, last_only=not watch)
        ...
        for checkpoint in checkpoints:
            ...
            tf.logging.info(
                "Evaluating global_step {} using checkpoint '{}'".format(checkpoint["global_step"], checkpoint["file"])
            )
            try:
                start = time.time()
                evaluate_once(
                ...
                tf.logging.info(
                    "Checkpoint {} is not ready yet. " "Checking again in a moment.".format(checkpoint["file"])
                )
                time.sleep(5)
                continue
...
def get_checkpoints(run_dir, from_global_step=None, last_only=False):
    ...
    checkpoints = sorted(
        [{"global_step": int(path.split("-")[-1]), "file": path} for path in ckpt.all_model_checkpoint_paths],
        key=lambda c: c["global_step"],
    )

    if last_only:
        checkpoints = checkpoints[-1:]
        tf.logging.info("Using last checkpoint in run_dir, global_step = {}".format(checkpoints[0]["global_step"]))
...
def evaluate_once(
    ...
    with tf.Session() as sess:
        ...
        try:
            ...
            # global step.
            ap_per_class, ar_per_class = calculate_metrics(output_per_batch, config.model.network.num_classes)
            ...
            tf.logging.info("Finished evaluation at step {}.".format(checkpoint["global_step"]))
            tf.logging.info("Evaluated {} images.".format(total_evaluated))

            tf.logging.info("Average Precision (AP) @ [0.50] = {:.3f}".format(map_at_50))
            tf.logging.info("Average Precision (AP) @ [0.75] = {:.3f}".format(map_at_75))
            tf.logging.info("Average Precision (AP) @ [0.50:0.95] = {:.3f}".format(map_at_range))
            tf.logging.info("Average Recall (AR) @ [0.50:0.95] = {:.3f}".format(mar_at_range))

            for idx, val in enumerate(ap_per_class[:, 0]):
                class_label = "{} ({})".format(class_labels[idx], idx) if class_labels else idx
                tf.logging.debug("Average Precision (AP) @ [0.50] for {} = {:.3f}".format(class_label, val))

            summary = [
                tf.Summary.Value(tag="{}/AP@0.50".format(metrics_scope), simple_value=map_at_50),
                tf.Summary.Value(tag="{}/AP@0.75".format(metrics_scope), simple_value=map_at_75),
                tf.Summary.Value(tag="{}/AP@[0.50:0.95]".format(metrics_scope), simple_value=map_at_range),
                tf.Summary.Value(tag="{}/AR@[0.50:0.95]".format(metrics_scope), simple_value=mar_at_range),
                tf.Summary.Value(tag="{}/total_evaluated".format(metrics_scope), simple_value=total_evaluated),
                tf.Summary.Value(tag="{}/evaluation_time".format(metrics_scope), simple_value=time.time() - start_time),
            ]
...
def calculate_metrics(output_per_batch, num_classes):
    ...
    """
    iou_thresholds = np.linspace(0.50, 0.95, int(np.round((0.95 - 0.50) / 0.05) + 1))
    # 101 recall levels, same as COCO evaluation.
    rec_thresholds = np.linspace(0.00, 1.00, int(np.round((1.00 - 0.00) / 0.01) + 1))
    ...
    for idx in range(num_batches):
        ...
        for cls in range(num_classes):
            ...
            if num_gt == 0:
                ...
                # be false positives.
                tp_fp_labels_by_class[cls].append((tp_fp_labels, cls_scores[sorted_indices]))
                continue
    ...
    for cls in range(num_classes):
        ...
        precision = np.divide(cum_true_positives.astype(float), cum_true_positives + cum_false_positives)
