...
def resize_image(image, bboxes=None, min_size=None, max_size=None):
    ...
    image = tf.image.resize_images(
        image, tf.stack(tf.to_int32([new_height, new_width])), method=tf.image.ResizeMethod.BILINEAR
    )

    if bboxes is not None:
        bboxes = adjust_bboxes(bboxes, old_height=height, old_width=width, new_height=new_height, new_width=new_width)
        return {
            "image": image,
            "bboxes": bboxes,
...
def patch_image(image, bboxes=None, offset_height=0, offset_width=0, target_height=None, target_width=None):
    ...
    if bboxes is None:
        ...
        new_image_resized = tf.image.resize_images(new_image, im_shape[:2], method=tf.image.ResizeMethod.BILINEAR)
    ...
    bboxes_center_y = tf.reduce_mean(tf.concat([bboxes[:, 1:2], bboxes[:, 3:4]], axis=1), axis=1)
    ...
    center_x_is_inside = tf.logical_and(
        tf.greater(bboxes_center_x, offset_width), tf.less(bboxes_center_x, tf.add(target_width, offset_width))
    )
    center_y_is_inside = tf.logical_and(
        tf.greater(bboxes_center_y, offset_height), tf.less(bboxes_center_y, tf.add(target_height, offset_height))
    )
    ...
    return_dict = {}
    return_dict["image"] = tf.cond(update_condition, lambda: new_image_resized, lambda: image)
    return_dict["bboxes"] = tf.cond(update_condition, lambda: new_bboxes_resized, lambda: bboxes)
    return return_dict
...
def random_patch(image, bboxes=None, min_height=600, min_width=600, seed=None):
    ...
    offset_width = tf.random_uniform(
        shape=[], minval=0, maxval=tf.subtract(im_shape[1], min_width), dtype=tf.int32, seed=seed
    )
    offset_height = tf.random_uniform(
        shape=[], minval=0, maxval=tf.subtract(im_shape[0], min_height), dtype=tf.int32, seed=seed
    )
    target_width = tf.random_uniform(
        shape=[], minval=min_width, maxval=tf.subtract(im_shape[1], offset_width), dtype=tf.int32, seed=seed
    )
    target_height = tf.random_uniform(
        shape=[], minval=min_height, maxval=tf.subtract(im_shape[0], offset_height), dtype=tf.int32, seed=seed
    )
    return patch_image(
        image,
        bboxes=bboxes,
...
def random_resize(image, bboxes=None, min_size=600, max_size=980, seed=None):
    ...
    if bboxes is not None:
        ...
        bboxes = adjust_bboxes(
            bboxes, old_height=im_shape[0], old_width=im_shape[1], new_height=new_size[0], new_width=new_size[1]
        )
...
def random_distortion(image, bboxes=None, brightness=None, contrast=None, hue=None, saturation=None, seed=None):
    ...
    if brightness is not None:
        ...
        image = tf.image.random_brightness(image, max_delta=brightness.max_delta, seed=seed)
    ...
    if contrast is not None:
        ...
        if "upper" not in contrast:
            contrast.upper = 1.2
        image = tf.image.random_contrast(image, lower=contrast.lower, upper=contrast.upper, seed=seed)
    if hue is not None:
    ...
    if saturation is not None:
        ...
        image = tf.image.random_saturation(image, lower=saturation.lower, upper=saturation.upper, seed=seed)
    if bboxes is None:
        return_dict = {"image": image}
    else:
        return_dict = {
...
def expand(image, bboxes=None, fill=0, min_ratio=1, max_ratio=4, seed=None):
    ...
    height = image_shape[0]
    width = image_shape[1]
    size_multiplier = tf.random_uniform([1], minval=min_ratio, maxval=max_ratio, seed=seed)
    ...
    paddings = tf.stack(
        [tf.concat([pad_top, pad_bottom], axis=0), tf.concat([pad_left, pad_right], axis=0), tf.constant([0.0, 0.0])]
    )
