...
def encode(bboxes, gt_boxes, variances=None):
    with tf.name_scope("BoundingBoxTransform/encode"):
        (bboxes_width, bboxes_height, bboxes_urx, bboxes_ury) = get_width_upright(bboxes)

        (gt_boxes_width, gt_boxes_height, gt_boxes_urx, gt_boxes_ury) = get_width_upright(gt_boxes)
