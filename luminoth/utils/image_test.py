...
from luminoth.utils.image import resize_image, flip_image, random_patch, random_resize, random_distortion, patch_image
...
class ImageTest(tf.test.TestCase):
    ...
    def _resize_image(self, image_array, boxes_array=None, min_size=None, max_size=None):
        ...
        if boxes_array is not None:
            ...
            boxes = None
        resized = resize_image(image, bboxes=boxes, min_size=min_size, max_size=max_size)
    ...
    def _flip_image(self, image_array, boxes_array=None, left_right=False, up_down=False, bboxes_dtype=tf.int32):
        image = tf.placeholder(tf.float32, image_array.shape)
        feed_dict = {
            image: image_array,
        }
        if boxes_array is not None:
            ...
            boxes = None
        flipped = flip_image(image, bboxes=boxes, left_right=left_right, up_down=up_down)
        with self.test_session() as sess:
    ...
    def testResizeOnlyImage(self):
        ...
        resized_image, _, scal = self._resize_image(self._gen_image(100, 1024, 3), min_size=0, max_size=2000)
        self.assertAllEqual(resized_image.shape, (100, 1024, 3))
        self.assertAlmostEqual(scale, 1.0)

        # max_size only, slightly smaller than origin image size.
        resized_image, _, scale = self._resize_image(self._gen_image(100, 1024, 3), max_size=1000)
        ...
        resized_image, _, scale = self._resize_image(self._gen_image(100, 1024, 3), min_size=120)
        self.assertAllEqual(resized_image.shape, (120, 1228, 3))
        self.assertAlmostEqual(int(scale * 100), 120)

        # max_size only, half image size
        resized_image, _, scale = self._resize_image(self._gen_image(100, 1024, 3), max_size=512)
        self.assertAllEqual(resized_image.shape, (50, 512, 3))
        self.assertAlmostEqual(scale, 0.5)

        # min_size only, double image size
        resized_image, _, scale = self._resize_image(self._gen_image(100, 1024, 3), min_size=200)
        ...
        resized_image, _, scale = self._resize_image(
            self._gen_image(100, 200, 3), min_size=int(100 * change), max_size=round(200 / change)
        )
        ...
        resized_image, _, scale = self._resize_image(self._gen_image(100, 200, 3), min_size=600, max_size=1000)
        self.assertAllEqual(resized_image.shape, (100 * 6, 200 * 6, 3))  # We end up bigger than maximum.
        self.assertAlmostEqual(scale, 6.0)

        resized_image, _, scale = self._resize_image(self._gen_image(2000, 600, 3), min_size=600, max_size=1000)
        self.assertAllEqual(resized_image.shape, (1000, 300, 3))  # We end up smaller than minimum image.
        self.assertAlmostEqual(scale, 0.5)
    ...
    def testPatchImageUpdateCondition(self):
        ...
        with self.test_session() as sess:
            ...
            # in it.
            patch = patch_image(
                image_ph, bboxes_ph, offset_height=45, offset_width=45, target_height=100, target_width=200
            )
    ...
    def testFlipImageBoxes(self):
        ...
        image = self._gen_image(100, 100, 3)
        boxes = np.array([[0, 0, 99, 99, -1]])
        flipped_image, flipped_boxes = self._flip_image(image, boxes, up_down=True, left_right=True)
        # Check that sum of columns is not modified, just the order.
        self.assertAllClose(boxes, flipped_boxes)
    ...
    def testFlipBboxesDiffDtype(self):
        ...
        _, flipped_boxes_float = self._flip_image(image, boxes, up_down=True, left_right=True, bboxes_dtype=tf.float32)

        _, flipped_boxes_int = self._flip_image(image, boxes, up_down=True, left_right=True, bboxes_dtype=tf.int32)
    ...
    def testRandomPatchImageBboxes(self):
        ...
        bboxes_w_label = tf.concat([bboxes, tf.fill((bboxes.shape[0], 1), label)], axis=1)
