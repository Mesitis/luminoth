...
def with_deprecation_warning(fn, extra_message=""):
    ...
    def new_fn(*args, **kwargs):
        if extra_message not in _DONE_WARN:
            tf.logging.warning(
                "Sonnet nest is deprecated. Please use " "tf.contrib.framework.nest instead. " + extra_message
            )
...
flatten_iterable = with_deprecation_warning(nest.flatten, "In addition, `flatten_iterable` is renamed to `flatten`.")
...
map = with_deprecation_warning(  # pylint: disable=redefined-builtin
    nest.map_structure, "In addition, `map` is renamed to `map_structure`."
)
