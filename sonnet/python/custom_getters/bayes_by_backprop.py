...
_OK_PZATION_TYPE = tf.distributions.FULLY_REPARAMETERIZED
...
def diagonal_gaussian_posterior_builder(getter, name, shape=None, *args, **kwargs):
    ...
    loc_var = getter(name + "/posterior_loc", shape=parameter_shapes["loc"], *args, **kwargs)
    scale_var = getter(
...
def fixed_gaussian_prior_builder(getter, name, dtype=None, *args, **kwargs):
    ...
    return tf.distributions.Normal(loc=loc, scale=scale, name="{}_prior_dist".format(name))
...
def bayes_by_backprop_getter(
    ...
    def custom_getter(getter, name, *args, **kwargs):
        ...
        if var_scope.reuse and not fresh_noise_per_connection:
            ...
            the_match = [x for x in get_variable_metadata() if x.raw_variable_name == name]
            if not the_match:
                raise ValueError("Internal error. No metadata for variable {}".format(name))
            if len(the_match) > 1:
                raise ValueError(
        ...
        def construct_subgraph():
            ...
            if posterior_dist.reparameterization_type != _OK_PZATION_TYPE:
                raise ValueError(
                    "Distribution {} incompatible with Bayes by Backprop.".format(posterior_dist.__class__.__name__)
                )

            posterior_estimator = _produce_posterior_estimate(posterior_dist, sampling_mode_tensor, name)
...
def get_variable_metadata(scope_name_substring=None):
    ...
    if scope_name_substring is not None:
        all_variable_metadata = [x for x in all_variable_metadata if scope_name_substring in x.scope_name]
    else:
