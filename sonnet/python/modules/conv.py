...
def _default_transpose_size(input_shape, stride, kernel_shape=None, padding=SAME):
    ...
    if not input_shape:
        raise TypeError(
            "input_shape is None; if using Sonnet, are you sure you " "have connected the module to inputs?"
        )
    ...
    if padding == VALID:
        kernel_shape = _fill_and_verify_parameter_shape(kernel_shape, input_length, "kernel")
...
def _fill_shape(x, n):
    ...
    raise TypeError(
        "x is {}, must be either a positive integer " "or an iterable of positive integers of size {}".format(x, n)
    )
...
def _fill_and_verify_parameter_shape(x, n, parameter_label):
    ...
    try:
        ...
        raise base.IncompatibleShapeError("Invalid " + parameter_label + " shape: " "{}".format(e))
...
def _verify_inputs(inputs, channel_index, data_format):
    ...
    if len(input_shape) != len(data_format):
        raise base.IncompatibleShapeError(
            (
                "Input Tensor must have rank {} corresponding to " "data_format {}, but instead was {} of rank {}."
            ).format(len(data_format), data_format, input_shape, len(input_shape))
        )

    # Check type.
    if not (tf.float16.is_compatible_with(inputs.dtype) or tf.float32.is_compatible_with(inputs.dtype)):
        raise TypeError("Input must have dtype tf.float16 or tf.float32, but dtype was {}".format(inputs.dtype))
...
def _find_channel_index(data_format):
    ...
    for i, c in enumerate(data_format):
        if c == "C":
            return i
    raise ValueError("data_format requires a channel dimension. Got: {}".format(data_format))
...
def _apply_bias(
    ...
    if "b" not in initializers:
        initializers["b"] = create_bias_initializer(bias_shape, dtype=inputs.dtype)
    b = tf.get_variable(
        "b",
        shape=bias_shape,
        dtype=inputs.dtype,
...
class _ConvND(base.AbstractModule):
    ...
    def __init__(
        ...
        self._kernel_shape = _fill_and_verify_parameter_shape(kernel_shape, self._n, "kernel")
        self._data_format = data_format

        # The following is for backwards-compatibility from when we used to accept
        # N-strides of the form [1, ..., 1].
        if isinstance(stride, collections.Iterable) and len(stride) == len(data_format):
        ...
        if mask is not None:
            if isinstance(mask, (tf.Tensor, list, tuple, np.ndarray)):
                ...
                if not (
                    tf.float16.is_compatible_with(self._mask.dtype)
                    or tf.float32.is_compatible_with(self._mask.dtype)
                    or tf.float64.is_compatible_with(self._mask.dtype)
                ):
                    raise TypeError("Mask needs to have dtype float16, float32 or float64")
                if not self._mask.shape.is_fully_defined():
                    base.IncompatibleShapeError("Mask needs to have a statically defined shape")
            else:
    ...
    def _construct_w(self, inputs):
        ...
        Returns:
          w: A weight matrix of the same type as `inputs`.
        """
        weight_shape = self._kernel_shape + (
            self._input_channels,
            self.output_channels,
        )

        if "w" not in self._initializers:
            self._initializers["w"] = create_weight_initializer(weight_shape[:-1], dtype=inputs.dtype)
    ...
    def _apply_mask(self):
        ...
        if self._mask.shape.ndims > w.shape.ndims:
            raise base.IncompatibleShapeError(
                "Invalid mask shape: {}. Max shape: {}".format(self._mask.shape.ndims, len(self._data_format))
            )
        if self._mask.shape != w.shape[: self._mask.shape.ndims]:
            raise base.IncompatibleShapeError(
                "Invalid mask shape: {}. Weight shape: {}".format(self._mask.shape, w.shape)
            )
...
class _ConvNDTranspose(base.AbstractModule):
    ...
    def __init__(
        ...
        Raises:
          ...
          ValueError: If the passed-in data_format doesn't have a channel dimension.
        """
        super(_ConvNDTranspose, self).__init__(custom_getter=custom_getter, name=name)
        self._data_format = data_format
        ...
        if output_shape is None:
            ...
            if callable(output_shape):
                ...
                self._output_shape = _fill_and_verify_parameter_shape(output_shape, self._n, "output_shape")
        ...
        if isinstance(stride, collections.Iterable) and len(stride) == len(data_format):
            if self._data_format.startswith("N") and self._data_format.endswith("C"):
                ...
                if not stride[0] == stride[1] == 1:
                    raise base.IncompatibleShapeError("Invalid stride: First and second element must be 1.")
            self._stride = tuple(stride)
        else:
            self._stride = _fill_and_one_pad_stride(stride, self._n, self._data_format)
    ...
    def _build(self, inputs):
        ...
        if self._use_default_output_shape:
            ...
            def _default_transpose_size_wrapper():
                ...
                return _default_transpose_size(
                    input_size,
                    stride,
                    kernel_shape=self._kernel_shape,
                    padding=self._padding,
        ...
        if len(self.output_shape) != self._n:
            raise base.IncompatibleShapeError(
                "Output shape must have rank {}, but instead was {}".format(self._n, len(self.output_shape))
            )
    ...
    def _construct_w(self, inputs):
        ...
        if self._n == 1:
            weight_shape = (1,) + self._kernel_shape + (self.output_channels, self._input_channels)
        else:
            weight_shape = self._kernel_shape + (
                self.output_channels,
                self._input_channels,
        ...
        if "w" not in self._initializers:
            ...
            self._initializers["w"] = create_weight_initializer(fan_in_shape, dtype=inputs.dtype)
        w = tf.get_variable(
            "w",
            shape=weight_shape,
            dtype=inputs.dtype,
...
class Conv1D(_ConvND, base.Transposable):
    ...
    def __init__(
        ...
        if data_format not in SUPPORTED_1D_DATA_FORMATS:
            raise ValueError(
                "Invalid data_format {:s}. Allowed formats " "{}".format(data_format, SUPPORTED_1D_DATA_FORMATS)
            )
    ...
    def transpose(self, name=None):
        ...
        return Conv1DTranspose(
            output_channels=lambda: self._input_channels,
...
class CausalConv1D(_ConvND):
    ...
    def __init__(
        ...
        if padding != VALID:
            raise base.NotSupportedError(
                "Causal padding requires padding argument to be VALID." "Got: {}".format(padding)
            )
...
class Conv3D(_ConvND, base.Transposable):
    ...
    def __init__(
        ...
        if data_format not in SUPPORTED_3D_DATA_FORMATS:
            raise ValueError(
                "Invalid data_format {:s}. Allowed formats " "{}".format(data_format, SUPPORTED_3D_DATA_FORMATS)
            )
...
class DepthwiseConv2D(_ConvND):
    ...
    def __init__(
        ...
        Raises:
          ...
          ValueError: If the passed-in data_format doesn't have a channel dimension.
        """
        if not isinstance(channel_multiplier, numbers.Integral) and not isinstance(channel_multiplier, tf.Dimension):
            raise ValueError(
                ("channel_multiplier ({}), must be of type " "(`tf.Dimension`, `numbers.Integral`).").format(
                    channel_multiplier
                )
            )
        if channel_multiplier < 1:
            raise ValueError("channel_multiplier ({}), must be >= 1".format(channel_multiplier))
        ...
        Returns:
          A tuple of two 4D Tensors, each with the same dtype as `inputs`:
            ...
            2. w_pw, the pointwise weight matrix, of shape:
              [1, 1, channel_multiplier * input_channels, output_channels].
        """
        depthwise_weight_shape = (1,) + self._kernel_shape + (self._input_channels, self._channel_multiplier)
        pointwise_input_size = self._channel_multiplier * self._input_channels
        pointwise_weight_shape = (
            1,
            1,
