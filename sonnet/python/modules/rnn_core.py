...
def _single_learnable_state(state, state_id=0, learnable=True):
    ...
    assert_rows_equal = [tf.assert_equal(s, unpacked_state[0]) for s in unpacked_state]
...
def trainable_initial_state(
    ...
    return nest.pack_sequence_as(structure=state_size, flat_sequence=flat_initial_state)
...
class TrainableInitialState(base.AbstractModule):
    ...
    def __init__(self, initial_state, mask=None, name="trainable_initial_state"):
        ...
        warnings.warn(
            "Use the trainable flag in initial_state instead.",
            DeprecationWarning,
            stacklevel=2,
        )
    ...
    def _build(self):
        ...
        if self._mask is not None:
            ...
            flat_learnable_state = [
                _single_learnable_state(state, state_id=i) for i, state in enumerate(flat_initial_state)
            ]

        return nest.pack_sequence_as(structure=self._initial_state, flat_sequence=flat_learnable_state)
