...
class BatchNormV2(base.AbstractModule):
    ...
    SUPPORTED_DATA_FORMATS = set.union(
        {"NC"},
    ...
    def _fused_batch_norm_op(self, input_batch, mean, variance, use_batch_stats):
        ...
        if len(self._data_format) == 4:
            ...
            fusable_batch = tf.reshape(input_batch, shape=(-1, self._num_channels, 1, flat_image_size))
        else:
            # The CPU implementation of FusedBatchNorm only supports NHWC tensor
            # format for now.
            fusable_data_format = "NHWC"
            fusable_batch = tf.reshape(input_batch, shape=(-1, 1, flat_image_size, self._num_channels))
    ...
    def _build(self, input_batch, is_training, test_local_stats=False):
        ...
        if not self._data_format:
            if len(input_shape) == 2:
                ...
                raise base.IncompatibleShapeError(
                    "Input shape {} has too many or too few dimensions.".format(input_shape)
                )
        ...
        if len(self._data_format) != len(input_shape):
            raise base.IncompatibleShapeError(
                "Incorrect data format {} for input shape {}.".format(self._data_format, input_shape)
            )
