...
class DeepRNN(rnn_core.RNNCore):
    ...
    def __init__(
        ...
        self._is_recurrent_list = [isinstance(core, rnn_core.RNNCore) for core in self._cores]

        if self._skip_connections:
            tf.logging.log_first_n(
                tf.logging.WARN,
                "The `skip_connections` argument will be deprecated. Please use " "snt.SkipConnectionCore instead.",
                1,
            )
        ...
        for core_sizes in zip(*tuple(_get_flat_core_sizes(self._cores))):
            ...
            for i, core_list in enumerate(core_sizes[1:]):
                if core_list[1:] != first_core_list:
                    raise ValueError(
                        "The outputs of the provided cores are not able "
    ...
    def _build(self, inputs, prev_state):
        ...
        for i, core in enumerate(self._cores):
            if self._skip_connections and i > 0:
                flat_input = (
                    nest.flatten(inputs),
                    nest.flatten(current_input),
                )
                flat_input = [tf.concat(input_, 1) for input_ in zip(*flat_input)]
                current_input = nest.pack_sequence_as(structure=inputs, flat_sequence=flat_input)

            # Determine if this core in the stack is recurrent or not and call
            # accordingly.
            if self._is_recurrent_list[i]:
                current_input, next_state = core(current_input, prev_state[recurrent_idx])
        ...
        if self._skip_connections and self._concat_final_output_if_skip:
            ...
            output = nest.pack_sequence_as(structure=outputs[0], flat_sequence=flat_outputs)
        else:
    ...
    def initial_state(
        ...
        if num_initializers != self._num_recurrent:
            raise ValueError(
                "The number of initializers and recurrent cores should "
                "be the same. Received %d initializers for %d specified "
                "recurrent cores." % (num_initializers, self._num_recurrent)
            )
    ...
    def output_size(self):
        if self._skip_connections and self._concat_final_output_if_skip:
            ...
            return nest.pack_sequence_as(structure=self._cores[0].output_size, flat_sequence=output_size)
        else:
            ...
            try:
                guessed_output_size = next(
                    core.output_size for core in reversed(self._cores) if hasattr(core, "output_size")
                )
