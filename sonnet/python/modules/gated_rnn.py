...
class LSTM(rnn_core.RNNCore):
    ...
    def __init__(
        ...
        self._partitioners = util.check_initializers(partitioners, self.possible_keys)
        self._regularizers = util.check_initializers(regularizers, self.possible_keys)
    ...
    @classmethod
    def get_possible_initializer_keys(cls, use_peepholes=False, use_projection=False):
    ...
    def _build(self, inputs, prev_state):
        ...
        if self._hidden_clip_value is not None:
            prev_hidden = tf.clip_by_value(prev_hidden, -self._hidden_clip_value, self._hidden_clip_value)
        if self._cell_clip_value is not None:
            prev_cell = tf.clip_by_value(prev_cell, -self._cell_clip_value, self._cell_clip_value)
    ...
    def state_size(self):
        ...
        return LSTMState(
            tf.TensorShape([self._hidden_state_size]),
...
class RecurrentDropoutWrapper(rnn_core.RNNCore):
    ...
    def __init__(self, core, keep_probs):
        ...
        super(RecurrentDropoutWrapper, self).__init__(custom_getter=None, name=core.module_name + "_recdropout")
        self._core = core
...
class ZoneoutWrapper(rnn_core.RNNCore):
    ...
    def __init__(self, core, keep_probs, is_training):
        ...
        super(ZoneoutWrapper, self).__init__(custom_getter=None, name=core.module_name + "_zoneout")
        self._core = core
        self._keep_probs = keep_probs
    ...
    def _build(self, inputs, prev_state):
        ...
        next_state = tf.contrib.framework.nest.map_structure(apply_zoneout, self._keep_probs, next_state, prev_state)
...
class BatchNormLSTM(rnn_core.RNNCore):
    ...
    def __init__(
        ...
        if max_unique_stats != 1 and not (use_batch_norm_h or use_batch_norm_x or use_batch_norm_c):
        ...
        if use_batch_norm_h:
            self._batch_norm_h = BatchNormLSTM.IndexedStatsBatchNorm(max_unique_stats, "batch_norm_h")
        if use_batch_norm_x:
            self._batch_norm_x = BatchNormLSTM.IndexedStatsBatchNorm(max_unique_stats, "batch_norm_x")
        if use_batch_norm_c:
            self._batch_norm_c = BatchNormLSTM.IndexedStatsBatchNorm(max_unique_stats, "batch_norm_c")
    ...
    def with_batch_norm_control(self, is_training, test_local_stats=True):
        ...
        """
        return BatchNormLSTM.CoreWithExtraBuildArgs(self, is_training=is_training, test_local_stats=test_local_stats)
    ...
    def _build(self, inputs, prev_state, is_training=None, test_local_stats=True):
        ...
        if self._use_batch_norm_h or self._use_batch_norm_x:
            ...
            if self._use_batch_norm_h:
                gates_h = self._gamma_h * self._batch_norm_h(gates_h, time_step, is_training, test_local_stats)
            if self._use_batch_norm_x:
                gates_x = self._gamma_x * self._batch_norm_x(gates_x, time_step, is_training, test_local_stats)
        ...
        if self._use_batch_norm_c:
            cell_output = self._beta_c + self._gamma_c * self._batch_norm_c(
                cell_output, time_step, is_training, test_local_stats
            )
        if self._use_peepholes:
            cell_output += self._w_o_diag * cell_output
        next_hidden = tf.tanh(cell_output) * tf.sigmoid(o)
    ...
    class CoreWithExtraBuildArgs(rnn_core.RNNCore):
        ...
        def __init__(self, core, *args, **kwargs):
            ...
            """
            super(BatchNormLSTM.CoreWithExtraBuildArgs, self).__init__(name=core.module_name + "_extra_args")
            self._core = core
            self._args = args
            self._kwargs = kwargs
...
class ConvLSTM(rnn_core.RNNCore):
    ...
    def _build(self, inputs, state):
        ...
        gates = tf.split(value=next_hidden, num_or_size_splits=4, axis=self._conv_ndims + 1)
...
class HighwayCore(rnn_core.RNNCore):
    ...
    def __init__(
        ...
        self._initializers = util.check_initializers(initializers, self.get_possible_initializer_keys(num_layers))
        self._partitioners = util.check_partitioners(partitioners, self.get_possible_initializer_keys(num_layers))
        self._regularizers = util.check_regularizers(regularizers, self.get_possible_initializer_keys(num_layers))
