...
ConnectedSubGraph = collections.namedtuple("ConnectedSubGraph", ("module", "name_scope", "inputs", "outputs"))
...
def _from_proto_sparse_tensor(sparse_tensor_proto, process_leafs):
    ...
    if not sparse_tensor_proto.HasField("named_tuple"):
        raise base_errors.ModuleInfoError("Error while deserializing a SparseTensor: expected proto tuple.")
    if sparse_tensor_proto.named_tuple.name != _SPARSE_TENSOR_NAME:
        raise base_errors.ModuleInfoError(
            ...
            "should have been {} but was {}.".format(_SPARSE_TENSOR_NAME, sparse_tensor_proto.named_tuple.name)
        )
...
_SpecialTypeInfo = collections.namedtuple("_SpecialTypeInfo", ("check", "to_proto", "from_proto"))
...
def _nested_to_proto(nested_value, nested_proto, process_leafs, already_processed):
    ...
    for type_name, type_info in six.iteritems(_TO_PROTO_SPECIAL_TYPES):
        if type_info.check(nested_value):
            ...
            type_info.to_proto(
                nested_value,
                nested_proto.special_type.object,
                process_leafs,
                already_processed,
            )
            return
...
def _module_info_from_proto(module_info_def, import_scope=None):
    ...
    for connected_subgraph_def in module_info_def.connected_subgraphs:
        connected_subgraph = ConnectedSubGraph(
            ...
            inputs=_nested_from_proto(connected_subgraph_def.inputs, process_leafs),
            outputs=_nested_from_proto(connected_subgraph_def.outputs, process_leafs),
        )
