...
class Linear(base.AbstractModule, base.Transposable):
    ...
    def _build(self, inputs):
        ...
        if len(input_shape) != 2:
            raise base.IncompatibleShapeError(
                "{}: rank of shape must be 2 not: {}".format(self.scope_name, len(input_shape))
            )

        if input_shape[1] is None:
            raise base.IncompatibleShapeError(
                "{}: Input size must be specified at module build time".format(self.scope_name)
            )

        if self._input_shape is not None and input_shape[1] != self._input_shape[1]:
            raise base.IncompatibleShapeError(
                "{}: Input shape must be [batch_size, {}] not: [batch_size, {}]".format(
                    self.scope_name, self._input_shape[1], input_shape[1]
                )
            )
    ...
    def transpose(self, name=None):
        ...
        return Linear(
            output_size=lambda: self.input_shape[1],
            use_bias=self._use_bias,
            initializers=self._initializers,
            partitioners=self._partitioners,
            regularizers=self._regularizers,
...
class AddBias(base.AbstractModule, base.Transposable):
    ...
    def _build(self, inputs, multiplier=1):
        ...
        if self._output_shape is not None and self._output_shape[1:] != input_shape[1:]:
            raise base.IncompatibleShapeError(
                "Input shape must be {} not: {}.".format(self._output_shape, input_shape[1])
            )
    ...
    def transpose(self, name=None):
        ...
        if name is None:
            name = self.module_name + "_transpose"
        return AddBias(
            output_shape=lambda: self._input_shape,
...
class BatchReshape(base.AbstractModule, base.Transposable):
    ...
    def _build(self, inputs):
        ...
        if len(full_input_shape) < self._preserve_dims:
            raise ValueError(
                "Input tensor has {} dimensions, should have at least "
                "as many as preserve_dims={}".format(len(full_input_shape), self._preserve_dims)
            )
        ...
        if len(self._input_shape) == 1 and len(self._shape) == 1:
            if self._shape[0] == -1 or self._shape[0] == self._input_shape[0]:
                ...
                if self._input_shape[0] is None:
                    raise ValueError(
                        "Unknown non-preserved dimensions are not allowed "
        ...
        if self._shape.count(-1) > 1:
            raise ValueError(
                "Wildcard -1 can appear only once in desired output shape. " "Given shape {}".format(self._shape)
            )
        ...
        if None in self._input_shape:
            raise ValueError(
                "Unknown non-preserved dimensions are not allowed in "
...
class TrainableVariable(base.AbstractModule):
    ...
    def __init__(
        ...
        Raises:
          ...
          TypeError: If any of the given regularizers are not callable.
        """
        super(TrainableVariable, self).__init__(custom_getter=custom_getter, name=name)
...
class MergeDims(base.AbstractModule):
    ...
    def _merge(self, tensor):
        ...
        if rank < start + self._size:
            raise ValueError("Rank of inputs must be at least {}.".format(self._start + self._size))
        ...
        if static_shape.count(None) + static_shape.count(0) <= 1:
            ...
            dynamic_middle = tf.reduce_prod(dynamic_input_shape[start : start + self._size], keep_dims=True)
            dynamic_final = dynamic_input_shape[start + self._size :]
            dynamic_shape = tf.concat([dynamic_initial, dynamic_middle, dynamic_final], axis=0)
    ...
    def _select(self, inputs, idx):
        if isinstance(idx, (list, tuple)):
            ...
            if idx < 0 or idx >= len(inputs):
                raise ValueError(
                    "`idx` contains out of bound entries (they should be " "in the range [0, {}))".format(len(inputs))
                )
