...
class AbstractModule(object):
    ...
    def __init__(self, _sentinel=None, custom_getter=None, name=None):  # pylint: disable=invalid-name
        ...
        if _sentinel is not None:
            raise ValueError("Calling AbstractModule.__init__ without named " "arguments is not supported.")

        if name is None:
            name = util.to_snake_case(self.__class__.__name__)
        elif not isinstance(name, six.string_types):
            raise TypeError("Name must be a string, not {} of type {}.".format(name, type(name)))
        ...
        self._custom_getter = _maybe_wrap_custom_getter(_variable_tracking_custom_getter, self._custom_getter)
    ...
    def _check_init_called(self):
        ...
        try:
            ...
            raise NotInitializedError(
                "You may have forgotten to call super at the " "start of %s.__init__." % self.__class__.__name__
            )
    ...
    def _set_module_info(self):
        ...
        self._module_info = base_info.ModuleInfo(
            ...
            class_name="{}.{}".format(self.__class__.__module__, self.__class__.__name__),
            connected_subgraphs=self._connected_subgraphs,
        )
        self._graph.add_to_collection(base_info.SONNET_COLLECTION_NAME, self._module_info)
    ...
    def _add_connected_subgraph(
        ...
        **inputs_kwargs,
        ...
        """
        build_inputs = inspect.getcallargs(call_method, *inputs_args, **inputs_kwargs)
        ...
        connected_subgraph = base_info.ConnectedSubGraph(
            module=self,
            name_scope=subgraph_name_scope,
            inputs=build_inputs,
            outputs=outputs,
        )
    ...
    def __call__(self, *args, **kwargs):
        ...
        self._add_connected_subgraph(self._build, outputs, subgraph_name_scope, *args, **kwargs)
        return outputs
    ...
    def _ensure_is_connected(self):
        ...
        if not self.is_connected:
            raise NotConnectedError(
                "Variables in {} not instantiated yet, __call__ the module " "first.".format(self.scope_name)
            )
    ...
    def get_all_variables(self, collection=tf.GraphKeys.TRAINABLE_VARIABLES):
        ...
        return tuple(
            sorted(
                self._all_variables & collection_variables,
                key=lambda v: v.name,
            )
        )
