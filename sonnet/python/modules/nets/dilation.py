...
def noisy_identity_kernel_initializer(base_num_channels, stddev=1e-8):
    ...
    def _noisy_identity_kernel_initializer(shape, dtype=tf.float32, partition_info=None):
        ...
        is_same_channel_multiple = tf.equal(
            tf.floordiv(
                _range_along_dimension(2, shape) * base_num_channels,
                in_channels,
            ),
            tf.floordiv(
                _range_along_dimension(3, shape) * base_num_channels,
                out_channels,
            ),
        )
        ...
        return tf.where(
            ...
            tf.ones(shape, dtype=dtype) * (base_num_channels / out_channels),
            noise,
        )
