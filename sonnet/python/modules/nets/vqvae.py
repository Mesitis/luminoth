...
class VectorQuantizer(base.AbstractModule):
    ...
    def _build(self, inputs, is_training):
        ...
        with tf.control_dependencies(
            [
                tf.Assert(
                    tf.equal(input_shape[-1], self._embedding_dim),
                    [input_shape],
                )
            ]
        ):
        ...
        quantized = tf.reshape(tf.matmul(encodings, self._w, transpose_b=True), tf.shape(inputs))
...
class VectorQuantizerEMA(base.AbstractModule):
    ...
    def _build(self, inputs, is_training):
        ...
        encoding_indices = tf.argmax(-distances, 1)
        encodings = tf.one_hot(encoding_indices, self._num_embeddings)
        quantized = tf.reshape(tf.matmul(encodings, w, transpose_b=True), tf.shape(inputs))
        e_latent_loss = tf.reduce_mean((tf.stop_gradient(quantized) - inputs) ** 2)

        if is_training:
            updated_ema_cluster_size = moving_averages.assign_moving_average(
                self._ema_cluster_size,
                tf.reduce_sum(encodings, 0),
                self._decay,
            )
