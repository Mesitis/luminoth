...
AttentionOutput = collections.namedtuple("AttentionOutput", ["read", "weights", "weight_logits"])
    ...
    def _build(self, memory, query, memory_mask=None):
        ...
        if inferred_memory_word_size is None or inferred_query_word_size is None:
            raise base.UnderspecifiedError(
                "memory_word_size and query_word_size must be known at graph " "construction time."
            )
