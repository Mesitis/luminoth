...
def _check_nested_callables(dictionary, object_name):
    ...
    for key, entry in six.iteritems(dictionary):
        if hasattr(entry, "items"):
            ...
            raise TypeError("{} for '{}' is not a callable function or dictionary".format(object_name, key))
...
def _assert_is_dictlike(maybe_dictlike, valid_keys):
    ...
    if not hasattr(maybe_dictlike, "__getitem__"):
        raise TypeError(
            "Expected a dict-like object with possible keys %s, received %s" % (str(valid_keys), str(maybe_dictlike))
        )
...
def custom_getter_router(custom_getter_map, name_fn):
    ...
    def _custom_getter(getter, name, *args, **kwargs):
        ...
        if num_matches == 0:
            ...
            raise KeyError(
                "More than one custom_getter matched {} ({}): {}".format(
...
def get_saver(
    scope,
    collections=(tf.GraphKeys.GLOBAL_VARIABLES,),  # pylint: disable=redefined-outer-name
    context=None,
    **kwargs,
):
...
def format_variable_map(variable_map, join_lines=True):
    ...
    sort_key = lambda item: (item[0], item[1].name)  # noqa: E731
...
def reuse_variables(method):
    ...
    def wrapper(obj, *args, **kwargs):
        ...
        variable_scope_context_manager = getattr(obj, "_enter_variable_scope", default_context_manager)
        ...
        with variable_scope_ops._pure_variable_scope(  # pylint:disable=protected-access
            variable_scope, reuse=reuse
        ) as pure_variable_scope:
...
def to_snake_case(camel_case):
    ...
    if not re.match(r"^[A-Za-z_]\w*$", camel_case):
        raise ValueError("Input string %s is not a valid Python identifier." % camel_case)
