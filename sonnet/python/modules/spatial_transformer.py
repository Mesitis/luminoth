    ...
    def __init__(self, source_shape, output_shape, num_coeff, name, **kwargs):
        ...
        if len(self._output_shape) > len(self._source_shape):
            raise base.Error(
                ...
                "smaller than source domain dimensionality ({})".format(
...
def _create_affine_features(output_shape, source_shape):
    ...
    """
    ranges = [np.linspace(-1, 1, x, dtype=np.float32) for x in reversed(output_shape)]
...
class AffineGridWarper(GridWarper):
    ...
    def _create_features(self, constraints):
        ...
        psi = _create_affine_features(output_shape=self._output_shape, source_shape=self._source_shape)
        scales = [(x - 1.0) * 0.5 for x in reversed(self._source_shape)]
        offsets = scales
    ...
    def _build(self, inputs):
        ...
        if number_of_params != self._constraints.num_free_params:
            raise base.Error(
                ...
                "definition: {} parameters expected, {} provided.".format(
            ...
            if self._psi[i] is not None:
                ...
                tiling_params = tf.concat(
                    [
                        batch_size,
                        tf.constant(1, shape=(1,)),
                        tf.ones_like(warped_coord.shape),
                    ],
                    0,
                )
    ...
    def inverse(self, name=None):
        ...
        if self._num_coeff != 6:
            raise tf.errors.UnimplementedError("AffineGridWarper currently supports" "inversion only for the 2D case.")
...
class AffineWarpConstraints(object):
    ...
    def __init__(self, constraints=((None,) * 3,) * 2):
        ...
        if any(len(x) != expected_num_cols for x in self._constraints):
            raise ValueError("The input list must define a Nx(N+1) matrix of " "contraints.")
