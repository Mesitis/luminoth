...
def _get_raw_data(subset):
    ...
    if raw_data is not None:
        ...
        train_data, valid_data, test_data, vocab = ptb_reader.ptb_raw_data(FLAGS.data_path)
        _LOADED.update(
            {
...
class PTB(object):
    ...
    def __call__(self):
        x_bm, y_bm = ptb_reader.ptb_producer(self.raw_data, self.batch_size, self.seq_len, name=self.name)
...
class CustomScaleMixture(object):
    def __init__(self, pi, sigma1, sigma2):
        self.mu, self.pi, self.sigma1, self.sigma2 = map(np.float32, (0.0, pi, sigma1, sigma2))
...
def lstm_posterior_builder(getter, name, *args, **kwargs):
    ...
    """
    del args
    parameter_shapes = tf.contrib.distributions.Normal.param_static_shapes(kwargs["shape"])

    # The standard deviation of the scale mixture prior.
    prior_stddev = np.sqrt(
        FLAGS.prior_pi * np.square(FLAGS.prior_sigma1) + (1 - FLAGS.prior_pi) * np.square(FLAGS.prior_sigma2)
    )
...
def build_modules(is_training, vocab_size):
    ...
    rnn_core = snt.DeepRNN(cores, skip_connections=False, name="deep_lstm_core")

    # Do BBB on weights but not biases of output layer.
    output_linear = snt.Linear(vocab_size, custom_getter={"w": non_lstm_bbb_custom_getter})
...
def build_logits(data_ops, embed_layer, rnn_core, output_linear, name_prefix):
    ...
    embedded_input_seq = snt.BatchApply(embed_layer, name="input_embed_seq")(data_ops.sparse_obs)
    ...
    output_logits = snt.BatchApply(output_linear, name="output_embed_seq")(rnn_output_seq)
...
def train(logdir):
    ...
    data_ops = ptb_train()
    embed_layer, rnn_core, output_linear = build_modules(is_training=True, vocab_size=ptb_train.vocab_size)
    prediction_logits, zero_state_op = build_logits(data_ops, embed_layer, rnn_core, output_linear, name_prefix="train")
    ...
    learning_rate = tf.get_variable(
        "lr",
        initializer=tf.constant(FLAGS.lr_start, shape=(), dtype=tf.float32),
    )
    ...
    predictions = tf.cast(tf.argmax(prediction_logits, axis=-1), tf.int32, name="pred")
    correct_prediction_mask = tf.cast(tf.equal(predictions, data_ops.sparse_target), tf.int32)
    accuracy = tf.reduce_mean(tf.cast(correct_prediction_mask, tf.float32), name="acc")
    error_rate = tf.subtract(1.0, accuracy, name="err")
    label_probs = tf.nn.softmax(prediction_logits, dim=-1)
    predictive_entropy = tf.reduce_mean(label_probs * tf.log(label_probs + 1e-12) * -1.0)
    ...
    with tf.train.MonitoredTrainingSession(is_chief=True, checkpoint_dir=logdir, save_summaries_secs=10) as sess:
        ...
        epoch_idx_start, step_idx_start = divmod(num_updates_v, ptb_train.num_batches)
        tf.logging.info("On start, epoch: {}\t step: {}".format(epoch_idx_start, step_idx_start))
        for epoch_idx in six.moves.range(epoch_idx_start, FLAGS.num_training_epochs):
            tf.logging.info("Beginning Epoch {}/{}".format(epoch_idx, FLAGS.num_training_epochs))
            tf.logging.info(
                (
            ...
            for _ in six.moves.range(ptb_valid.num_batches):
                valid_cost_v, num_updates_v = _run_session_with_no_hooks(sess, [valid_loss, global_step])
                valid_cost += valid_cost_v
                valid_steps += FLAGS.unroll_steps
            tf.logging.info("Validation set perplexity: {}".format(np.exp(valid_cost / valid_steps)))
            ...
            for batch_idx in six.moves.range(step_idx_start, ptb_train.num_batches):
                scalars_res, num_updates_v = sess.run([log_ops_to_run["scalar"], global_step_and_train])
                ...
                if (batch_idx - 1) % FLAGS.print_every_batches == 0:
                    ...
                    scalars_res, strings_res = _run_session_with_no_hooks(
                        sess,
                        [log_ops_to_run["scalar"], log_ops_to_run["text"]],
                    )
            ...
            tf.logging.info("Train Perplexity after Epoch {}: {}".format(epoch_idx, word_level_perplexity))
...
def test(logdir):
    ...
    embed_layer, rnn_core, output_linear = build_modules(is_training=False, vocab_size=ptb_test.vocab_size)
    prediction_logits, _ = build_logits(data_ops, embed_layer, rnn_core, output_linear, name_prefix="test")
    ...
    with tf.train.SingularMonitoredSession(checkpoint_dir=logdir) as sess:
        tf.logging.info("Running on test set in {} batches.".format(ptb_test.num_batches))
        tf.logging.info("The model has trained for {} steps.".format(_run_session_with_no_hooks(sess, global_step)))
        ...
        tf.logging.info("Final test set perplexity: {}.".format(np.exp(dataset_cost / dataset_iters)))
...
def main(unused_argv):
    ...
    if FLAGS.mode == "train_only":
        ...
        tf.logging.info("Beginning a training phase of {} epochs.".format(FLAGS.num_training_epochs))
        train(logdir)
        ...
        raise ValueError(
            "Invalid mode {}. Please choose one of {}.".format(FLAGS.mode, "['train_only', 'test_only', 'train_test']")
        )
