...
SequenceDataOpsNoMask = collections.namedtuple("SequenceDataOpsNoMask", ("obs", "target"))
...
class TinyShakespeareDataset(snt.AbstractModule):
    ...
    VALID = "valid"
    TEST = "test"
    _RESOURCE_ROOT = os.path.join(os.path.dirname(os.path.realpath(__file__)), "data")
    ...
    def __init__(
        ...
        if subset not in [self.TRAIN, self.VALID, self.TEST]:
            raise ValueError(
                "subset should be %s, %s, or %s. Received %s instead." % (self.TRAIN, self.VALID, self.TEST, subset)
            )
        ...
        self._vocab_file = gfile.Open(os.path.join(self._RESOURCE_ROOT, "ts.train.txt"))
        self._data_file = gfile.Open(os.path.join(self._RESOURCE_ROOT, "ts.{}.txt".format(subset)))
        self._num_steps = num_steps
        self._batch_size = batch_size
        self._random_sampling = random
        self._dtype = dtype

        self._data_source = TokenDataSource(data_file=self._data_file, vocab_data_file=self._vocab_file)
    ...
    def _reset_head_indices(self):
        self._head_indices = np.random.randint(low=0, high=self._n_flat_elements, size=[self._batch_size])
    ...
    def _get_batch(self):
        ...
        batch_indices = np.mod(
            np.array([np.arange(head_index, head_index + self._num_steps + 1) for head_index in self._head_indices]),
            self._n_flat_elements,
        )

        obs = np.array([self._flat_data[indices[: self._num_steps]] for indices in batch_indices]).T
        target = np.array([self._flat_data[indices[1 : self._num_steps + 1]] for indices in batch_indices]).T

        if self._random_sampling:
            self._reset_head_indices()
        else:
            self._head_indices = np.mod(self._head_indices + self._num_steps, self._n_flat_elements)
        return obs, target
    ...
    def to_human_readable(self, data, label_batch_entries=True, indices=None, sep="\n"):
        ...
        result = []
        indices = range(batch_size) if not indices else indices
        for b in indices:
