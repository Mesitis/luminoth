...
tf.flags.DEFINE_integer("num_training_iterations", 10000, "Number of iterations to train for.")
...
tf.flags.DEFINE_float("optimizer_epsilon", 0.01, "Epsilon used for Adam optimizer.")
tf.flags.DEFINE_string("checkpoint_dir", "/tmp/tf/rnn_shakespeare", "Checkpointing directory.")
tf.flags.DEFINE_integer("checkpoint_interval", 500, "Checkpointing step interval.")
...
def build_graph(
    ...
    reduce_learning_rate_multiplier=0.1,
    ...
    train_loss = dataset_train.cost(train_output_sequence_logits, train_target_sequence)
    ...
    valid_loss = dataset_valid.cost(valid_output_sequence_logits, valid_target_sequence)
    ...
    test_loss = dataset_test.cost(test_output_sequence_logits, test_target_sequence)
    ...
    grads, _ = tf.clip_by_global_norm(tf.gradients(train_loss, trainable_variables), max_grad_norm)
    ...
    reduce_learning_rate = learning_rate.assign(learning_rate * reduce_learning_rate_multiplier)
    ...
    optimizer = tf.train.AdamOptimizer(learning_rate, epsilon=optimizer_epsilon)
...
def train(num_training_iterations, report_interval, reduce_learning_rate_interval):
    ...
    saver_hook = _configure_saver(FLAGS.checkpoint_dir, FLAGS.checkpoint_interval)

    # Train the network.
    with tf.train.SingularMonitoredSession(hooks=[saver_hook], checkpoint_dir=FLAGS.checkpoint_dir) as sess:
        ...
        for train_iteration in range(start_iteration, num_training_iterations):
            if (train_iteration + 1) % report_interval == 0:
                ...
                train_generated_string_v = sess.run(graph_tensors["train_generated_string"])
                ...
                tf.logging.info(
                    "%d: Training loss %f. Validation loss %f. Sample = %s",
                    train_iteration,
                    train_loss_v,
                    valid_loss_v,
                ...
                train_loss_v, _ = sess.run((graph_tensors["train_loss"], graph_tensors["train_step"]))
...
class TextModel(snt.AbstractModule):
    ...
    def __init__(
        ...
        with self._enter_variable_scope():
            ...
            self._subcores = [snt.LSTM(self._num_hidden, name="lstm_{}".format(i)) for i in range(self._lstm_depth)]
            ...
            self._core = snt.DeepRNN(self._subcores, skip_connections=False, name="deep_lstm")
